...
from .utils import debug, USER_AGENTS, input_generator, resolve_ips
...
class HTTPZScanner:
    ...
    def __init__(
        ...
        favicon_max_size  = DEFAULT_FAVICON_BYTES,
        ...
        :param max_body_size: byte cap on response body read
        :param favicon_max_size: byte cap on favicon read
        ...
        self.favicon_max_size = favicon_max_size
    ...
    async def _check_url(self, session: aiohttp.ClientSession, url: str, protocol: str, domain: str) -> dict:
        ...
        try:
            async with session.request(
                ...
                if self._needs_body:
                    try:
                        raw_body = await response.content.read(self.max_body_size)
                ...
                if raw_body is not None:
                    ...
                    if self.fetch_title or self.fetch_body or self.fetch_favicon:
                        ...
                        if self.fetch_favicon:
                            ...
                            fav    = await get_favicon_hash(session, base, html_text, max_bytes=self.favicon_max_size, timeout=self.timeout)
                            if fav:
    ...
    async def check_domain(self, session: aiohttp.ClientSession, domain: str) -> dict:
        ...
        ips_task = None
        if self.fetch_ips:
            ips_task = asyncio.create_task(resolve_ips(base_domain, self.resolvers, self.dns_timeout))
        ...
        for protocol in protocols:
            ...
            if port:
                url += f':{port}'
            result = await self._check_url_with_retries(session, url, protocol, base_domain)
            if result.get('status', -1) >= 0:
                success = result
                break
            last_error = result
        ...
        if ips_task is not None:
            try:
                ...
                debug(f'IP resolve error for {base_domain}: {e}')
